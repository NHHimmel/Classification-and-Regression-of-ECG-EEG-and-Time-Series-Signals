...
train_dl, val_dl, test_dl = load_dataset(
    dataset_name="appliances_energy", base_path=BASE_PATH,
...
for model_name in MODELS:
    ...
    # Evaluation
    avg_loss, nrmse = reg.evaluate(test_dl)
    results.append((model_name, avg_loss, nrmse))
...
print(f"{'Model':<8} | {'Test Loss':>10} | {'NRMSE':>8}")
print("-" * 48)
for name, loss, nrmse in results:
    print(f"{name.upper():<8} | {loss:>10.4f} | {nrmse:>8.4f}")

best_name, best_loss, best_nrmse = min(results, key=lambda r: r[2])
print("-" * 48)
print(f"Best model: {best_name.upper()} (NRMSE: {best_nrmse:.4f}, loss: {best_loss:.4f})")
